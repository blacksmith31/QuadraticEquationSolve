...
        char [] g = new char[dim]; //character array for open space/y-axis
        char [] h = new char[dim]; //character array for x-axis
        char open = ' ';
        char yAx = '|';
        char xAx = '-';
        char pt = '*';

        for(int i = 0; i<dim; i++){ // i iterator prints each line        
		Arrays.fill(g, open);
                	g[10] = yAx;
			Arrays.fill(h, xAx);              
			if(i==10){                   
...
                   
				...
				System.out.print(h);  // at i=10 (11th line) x axis is printed                  
...
        }
		System.out.println(" "); //print space after end of graph

    } //end printGraph
   
/*************************************************************************/

public Double [] quadSolve(){ //solve for x-intercepts
	double b2 = Math.pow(b, 2)-4*a*c;
    	if(b2 < 0){
			Double bN = null;
			return new Double [] {bN};
		}
		else if(b2 == 0){
...
        double [] coeffArr = new double[3];
        System.out.println("Enter A, B, and C coefficients for the format \nAx^2 + Bx + C = y, one on each line of input box");
        Scanner coIn = new Scanner(System.in);
        for(int m = 0; m < 3; m++) {
			coeffArr[m] = coIn.nextDouble();
        } //end fill coefficient array
        
		double coA = coeffArr[0];
		double coB = coeffArr[1];
        double coC = coeffArr[2];

        QuadGraph plot1 = new QuadGraph(coA, coB, coC);    
		QuadGraph.yVal();
		plot1.printGraph();
